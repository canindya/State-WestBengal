...
- **Session 4** (2026-04-12): Visual refresh inspired by Kolkata City Dashboard + numeric safety + mobile polish. Created `src/lib/format.ts` with safe numeric helpers (return `—` for null/NaN). Rebuilt landing page with magazine-style editorial hero, seasonal tagline, 4 hero narrative cards ("Growing/Welcoming/Building/Remembering"), and thematic section groups (People & Society / Economy & Business / Culture & Tourism / Environment & Climate / Geographic). Moved navbar desktop breakpoint `lg` → `xl` to fit 14 nav items without overflow. Softened card radii to `rounded-2xl`, added `tabular-nums` + `uppercase tracking-wide` labels, enlarged PageHeader h1s. Tightened YAxis label widths on 7 vertical bar charts for mobile (140-180px → 100-130px). Build passes all 14 routes; zero `NaN`/raw numbers detected in rendered HTML. Theme tokens already matched City-Kolkata hex-for-hex, so no color changes needed.
